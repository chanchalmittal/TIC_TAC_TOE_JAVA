...
{
    public static int j;
    private final JPanel mousetrap = new JPanel();
    int[][] A =new int[3][3];
    int count=-1;
    int count1=1;
    char Winner =  ' ';
    ...
    public TicPane()
    {
        ...
        mousetrap.setBackground(Color.WHITE);
        add(mousetrap, BorderLayout.CENTER);

        Handlerclass handler=new Handlerclass();
        mousetrap.addMouseListener(handler);
    }
        ...
        public void mouseClicked(MouseEvent event)
        {
            try
            {
                if(count1==1)
                    ++count;
                count1++;
                Graphics g= mousetrap.getGraphics();
                int n=event.getX();
                int m=event.getY();
    ...
    public void End() {
        if(Winner=='X')
        {
            JOptionPane.showMessageDialog(null, "CONGRATULATIONS X YOU ARE THE WINNER !","WINNER", JOptionPane.PLAIN_MESSAGE);
            System.exit(0);
        }

        else if(Winner=='O')
        {
            JOptionPane.showMessageDialog(null, "CONGRATULATIONS O YOU ARE THE WINNER!","WINNER", JOptionPane.PLAIN_MESSAGE);
            System.exit(0);
        }
        else if(Winner=='D')
        {
            JOptionPane.showMessageDialog(null, "OOPS! It's a draw!","WINNER", JOptionPane.PLAIN_MESSAGE);
            System.exit(0);
        }
    }
